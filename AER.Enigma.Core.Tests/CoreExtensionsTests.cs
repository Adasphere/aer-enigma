    ...
    public class CoreExtensionsTests
    {
        ...
        public void ToNullableDoubleTest()
        {
            for (int i = 0; i < UnitTestHelpers.TestRecurrence; ++i)
            {
                ...
                Assert.IsNull(
                    actual,
                    $"{nameof(CoreExtensions.ToNullableDouble)} did not return null when passed null");
                actual = FailureString.ToNullableDouble();
                Assert.IsNull(
                    actual,
                    $"{nameof(CoreExtensions.ToNullableDouble)} did not return null when passed a garbage string");

                actual = $"1.0E{UnitTestHelpers.Next(350, 400)}".ToNullableDouble();
                Assert.IsNull(
                    actual,
                    $"{nameof(CoreExtensions.ToNullableDouble)} did not return null when passed a number greater than Double.MaxValue {actual}");

                actual = $"-1.0E{UnitTestHelpers.Next(400, 500)}".ToNullableDouble();
                Assert.IsNull(
                    actual,
                    $"{nameof(CoreExtensions.ToNullableDouble)} did not return null when passed a number less than Double.MinValue");

                // test is written a little obtusely due to trouble with comparing double values due to rounding.
                string expected = $"1E+{UnitTestHelpers.Next(100, 200)}";
                actual = expected.ToNullableDouble();
                Assert.AreEqual(
                    expected,
                    actual.ToString(),
                    $"Actual result: {actual} did not match expected result: {expected}");
            }
        }
        ...
        public void ToNullableIntTest()
        {
            for (int i = 0; i < UnitTestHelpers.TestRecurrence; ++i)
            {
                ...
                Assert.IsNull(
                    actual,
                    $"{nameof(CoreExtensions.ToNullableint)} did not return null when passed a garbage string");

                actual = long.MaxValue.ToString().ToNullableint();
                Assert.IsNull(
                    actual,
                    $"{nameof(CoreExtensions.ToNullableint)} did not return null when passed a number greater than Int32.MaxValue");

                actual = long.MinValue.ToString().ToNullableint();
                Assert.IsNull(
                    actual,
                    $"{nameof(CoreExtensions.ToNullableint)} did not return null when passed a number less than Int32.MinValue");

                double? expected = UnitTestHelpers.Next();
